...
工具返回的 `output` 已经是面向用户的最终 Markdown。完整交付该结果后立即停止，禁止添加开场白、完成说明、摘要、结尾或自己的判断；不得改写、删减、重排或翻译。结论中只保留 `〔n〕` 短引用，不得把 Evidence ID 复制到结论；完整 Evidence ID 只保留在编号证据卡片的溯源字段中。必须保留全部标题、短引用、知识入口、编号证据卡片、Wiki HTTP 链接、Evidence ID、证据摘录、完整表格、OpenCode 内原图、图片解析来源、公式、回退信息和运行信息。
